...
Each Recent Call can be expanded on **Activity** into a focused call thread: its consented recap, call-linked lifecycle/SMS events, and notes written specifically about that call. The URL may include `/activity?call=<call-id>` for a refreshable, shareable-in-session view. The standalone Timeline contains only unlinked person-level activity, so call events are not duplicated in a long global feed.
...
Care-circle Notes are separate, intentionally shared dashboard updates. Operators and trusted contacts with the `care_notes` permission can add immutable general notes or, with `view_summaries`, attach a note to a specific call thread; corrections are new notes. General notes appear on Home, while call-specific notes remain in their thread. Notes update the dashboard’s last check-in date; viewers who also have `view_summaries` see completed Iris calls in that date as well. Notes never become Iris memory, Bridge context, phone-session instructions, Timeline events, or audit metadata. Existing dashboard links issued before Notes was added do not receive `care_notes`; regenerate a link to grant Notes access.
...
7. Open **Activity**, expand the completed call, and confirm its recap plus linked call lifecycle/Bridge/Shield events appear only in that thread. Refresh the page with `/activity?call=<call-id>` and confirm the same call reopens. The standalone Timeline should retain only unlinked person-level activity.
8. In that expanded call, add a call-specific note as the operator and confirm it appears in the thread but not Home Notes. On the homepage, add a general **Care-circle note** and confirm it appears immediately with the author and timestamp, and that **Last check-in** advances. Open a newly generated trusted-contact link with both `care_notes` and `view_summaries`; add a second call-specific note and confirm both viewers see it. A legacy link without `care_notes` must show neither Notes nor last check-in; regenerate it to grant access.
...
10. Check that no recap card, thread, Notes card, or Timeline payload exposes a recall anchor, raw transcript, SMS body, phone number, provider identifier, Shield scenario, red-flag label, or a trusted contact’s internal ID. Notes must not change Iris’s spoken continuity on a later call.
...
Raw phone audio, raw transcripts, message bodies, phone numbers, and provider identifiers are never rendered in the dashboard timeline. Phone transcripts are never persisted; they remain in memory through consent-gated summary extraction after the call, then are discarded. Conversation-derived durable storage is limited to consented structured summaries, user-stated facts, named people/context, unresolved topics, and recall anchors. The dashboard receives only the separately consented shared care recap, including explicitly discussed health-related concerns and clearly attributed Iris suggestions; private memory fields and recall anchors never reach the dashboard. Care-circle notes are deliberately shared, append-only dashboard content for viewers with `care_notes`; they are never inserted into Iris’s memory or phone context. Required operational audit and outbox records may also be retained.
