...
Each Recent Call can be expanded on **Activity** into a focused call thread: its consented recap, call-linked lifecycle/SMS events, and notes written specifically about that call. The URL may include `/activity?call=<call-id>` for a refreshable, shareable-in-session view. **Other Events** contains the operator SMS recovery queue and the standalone Timeline, which contains only unlinked person-level activity so call events are not duplicated in a long global feed.
...
Care-circle Notes are separate, intentionally shared dashboard updates. The Home form creates general person-level notes. With both `care_notes` and `view_summaries`, an expanded call offers a separate form that creates a note attached to that call. Home includes general notes plus notes from exactly the newest visible call; older call notes stay in their own thread. Authors may edit or soft-delete their own notes: the shared operator identity owns operator notes, while a trusted contact owns only notes tied to their live contact record. Deleted notes remain internal but disappear from dashboard views and last-check-in calculations. Notes never become Iris memory, Bridge context, phone-session instructions, Timeline events, or audit metadata. Existing dashboard links issued before Notes was added do not receive `care_notes`; regenerate a link to grant Notes access.
...
7. Open **Activity**, expand the completed call, and confirm its recap plus linked call lifecycle/Bridge/Shield events appear only in that thread. Refresh the page with `/activity?call=<call-id>` and confirm the same call reopens. Open **Other Events** and confirm the unlinked Timeline and operator SMS recovery controls are there instead of on Activity.
8. Open the newest visible call in **Activity**. Add a call-specific note as the operator and confirm it appears in the thread and on Home because this is the newest visible call. Add a general **Care-circle note** from Home and confirm it remains a person-level update. Edit and delete each as its author; confirm deleted notes disappear from the thread, Home, and **Last check-in** calculation. Add a note to an older call and confirm it stays thread-only on Home. Open a newly generated trusted-contact link with both `care_notes` and `view_summaries`; add a second call-specific note and confirm both viewers see it, while a different trusted contact cannot edit or delete it. A legacy link without `care_notes` must show neither Notes nor last check-in; regenerate it to grant access.
