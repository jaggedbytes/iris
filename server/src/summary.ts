...
function extractionInstructions(careSharingActive: boolean) {
  ...
  const care = careSharingActive
    ? "careSummary is dashboard-only. Write a concise recap whenever the person shares a meaningful, everyday, safe update, even if they did not express a concern and Iris gave no next step. moodAndConcerns and irisSuggestedNextSteps may be empty. Include mood or concern only when the person explicitly stated it. Include an Iris next step only when Iris clearly said it, and attribute it as an Iris suggestion. Never include symptoms, diagnoses, medications, treatment, appointments, providers, insurance, care needs, legal matters, financial or payment details, account information, credentials, passcodes, SSNs, payment-card numbers, phone numbers, email addresses, physical addresses, dates of birth, or assistant inference. Do not classify risk. Set careSummary to null only when there is no meaningful safe update or its content is unsafe to share."
