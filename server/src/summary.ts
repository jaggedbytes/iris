...
function extractionInstructions(careSharingActive: boolean) {
  ...
  const care = careSharingActive
    ? "careSummary is dashboard-only and must be a concise recap plus explicitly user-stated mood or concern and concrete non-expert Iris next steps. Attribute each next step as an Iris suggestion. Never include symptoms, diagnoses, medications, treatment, appointments, providers, insurance, care needs, legal matters, financial or payment details, account information, credentials, passcodes, SSNs, payment-card numbers, phone numbers, email addresses, physical addresses, dates of birth, or assistant inference. Do not classify risk. If there is no safe care signal, set careSummary to null."
