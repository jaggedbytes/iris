      ...
      try {
        ...
        const instructions = [
          ...
          this.checkInRequesterDisplayName
            ...
            : "",
          this.bridge
            ? `Bridge context:\n${this.bridge.context}\n\nAuthoritative phone-session instructions: the configured bridge_send_sms tool is allowed only after the person clearly says yes to sending a specific message to a listed trusted contact. First say who you would contact and the exact final text, including the Iris prefix and HELP/STOP footer, then ask for approval. Never call it on ambiguity. Keep the message content at or below ${MAX_SMS_CONTENT_LENGTH} characters so the required prefix and footer fit; never promise text beyond that limit. ${this.bridge.recallOpener ? `After any family-requested greeting, offer exactly one gentle invitation based on this prior user-stated thread: ${JSON.stringify(this.bridge.recallOpener)}. Do not present it as certain, and do not repeat it later in the call.` : "Do not volunteer prior conversation details at the opening of this call."}`
            : "",
          this.shield
