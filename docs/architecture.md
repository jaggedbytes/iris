...
- Care-circle notes are append-only dashboard content. General notes are person-scoped and appear on Home; notes created from an expanded call are linked to that call and appear only in its Activity thread. The `care_notes` scope grants general Notes access, while call-thread notes additionally require `view_summaries`; every note records an operator or trusted-contact display-name/relationship snapshot so attribution survives contact deletion. Notes are never Timeline events, audit metadata, private Iris memory, Bridge context, or phone-session instructions. A viewer without `care_notes` receives neither notes nor the Notes-card last check-in date; existing links must be regenerated to gain the new scope.
...
- Timeline and call-thread payloads are allowlisted: no SMS body, phone number, provider ID, raw transcript, or audit metadata reaches the browser. The global Timeline contains only unlinked person-level events; call-linked events appear only in their Recent Call thread.
