...
- Timeline and call-thread payloads are allowlisted: no SMS body, phone number, provider ID, raw transcript, or audit metadata reaches the browser. **Activity** is limited to Recent Call threads; **Other Events** contains the operator action/recovery queue and the global Timeline. That Timeline contains only unlinked person-level events; call-linked events appear only in their Recent Call thread.
